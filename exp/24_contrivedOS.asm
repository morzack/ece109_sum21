...
TRAP_BSOD
        ST R0, OS_SAVE_R0  ; save registers
        ST R1, OS_SAVE_R1
        ST R2, OS_SAVE_R2
        LD R0, BSOD_START ; load starting point of bsod
        LD R2, TOPLT    ; load starting point of display
BSODRPT LDR R1, R0, #0  ; load value from memory
        STR R1, R2, #0  ; store to display
        ADD R0, R0, #1  ; increment bsod memory address
        ADD R2, R2, #1  ; increment display memory address
        LD R1, N_BOTRT  ; load negated bottom right of display to compare
        ADD R1, R1, R2  ; perform comparison
        BRn BSODRPT     ; repeat if not at the end of display
        LD R0, OS_SAVE_R0 ; restore registers
        LD R1, OS_SAVE_R1
        LD R2, OS_SAVE_R2
        RTT             ; return
...
WELCOME_MSG     .STRINGZ "Welcome to contrivedOS!\n"
HALT_MSG        .STRINGZ "\nProcessor halted.\n"
BSOD_START      .FILL x4000
TOPLT           .FILL xC000
N_BOTRT         .FILL x0201
