;
; draw_lines.asm - draw horizontal/vertical lines for graphics display
;
...
; ------------------------------- MAIN ROUTINE ------------------------------- ;
        ...
        LD R3, TOPLT    ; load top left xC000 - row
        LD R4, TOPLT    ; load top left xC000 - col

LOOP    LD R5, N_BOTLT  ; load negated bottom left -xFD80
        ADD R5, R3, R5  ; compare to see if at the bottom
        BRp SKIP        ; end of rows, skip over drawing row
        JSR RANDOM      ; get random color - from subroutine
        ADD R1, R3, #0  ; move row number to R1
        JSR H_LINE      ; draw a row

SKIP    LD R5, N_TOPRT  ; load negated top right -xC07F
        ADD R5, R4, R5  ; compare to see if at the end
        BRp END         ; end of cols, skip to END
        JSR RANDOM      ; get random color - from subroutine
        ADD R1, R4, #0  ; move col number to R1
        JSR V_LINE      ; draw a column

        LD R5, WIDTH    ; load width to add to increment row memory address
        ADD R3, R3, R5  ; increment row
        ADD R4, R4, #1  ; increment column
        BRnzp LOOP      ; unconditionally repeat
...
H_LINE
        ...
        LD R4, WIDTH    ; load width to act as decrementing counter
H_LOOP  STR R2, R1, #0  ; write the color to the next memory address
        ADD R1, R1, #1  ; compute next memory address by adding 1
        ADD R4, R4, #-1 ; decrement width counter
        BRp H_LOOP      ; repeat if still pixels left in horizontal line
...
V_LINE
        ...
        LD R3, WIDTH    ; load the width to add as an offset
        LD R4, HEIGHT   ; load height to act as decrementing counter
V_LOOP  STR R2, R1, #0  ; write the color to the next memory address
        ADD R1, R1, R3  ; compute next memory address by adding the width
        ADD R4, R4, #-1 ; decrement height counter
        BRp V_LOOP      ; repeat if still pixels left in vertical line
